...
def solution(xs):
    ...
    for x in xs:
        if x == 0: continue
        elif x > 0: positive.append(x)
        else: negative.append(x)
    
    negative.sort(reverse=True)
    if len(negative) % 2 == 1: negative = negative[1:]

    if len(positive) == 0 and len(negative) == 0: return "0"

    ans = 1
    for x in positive: ans *= x
    for x in negative: ans *= x
